import logging, requests, urllib.parse, os, time, shutil, asyncio, json, math, re
...
@Clinton.on_message(filters.private & ~filters.via_bot & filters.regex(pattern="(?P<url>https?://[^\s]+)"))
async def echo(bot, update):
    ...
    if "|" in url:
        ...
        for entity in update.entities:
            if entity.type == "text_link":
                ...
                url = url[o:o + l]
    if url is None:
         url = re.search("(?P<url>https?://[^\s]+)", update.text).group("url")
    ...
    if t_response:
        ...
        if "formats" in response_json:
            ...
            await bot.send_message(
            chat_id=update.chat.id,
            text=Translation.NO_VOID_FORMAT_FOUND.format("No formats found"),
            disable_web_page_preview=True, parse_mode="html",
            reply_to_message_id=update.message_id
        )
    else:
